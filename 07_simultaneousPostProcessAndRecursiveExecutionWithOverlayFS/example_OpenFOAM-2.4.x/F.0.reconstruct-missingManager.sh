...
#SBATCH --job-name=reconstruct-MissingManager
...
i=0
surnamesList[0]=""
surnamesSubmitted[0]="false" #Initialised in false, updated when submission, checked at the end
((i++))
for jj in $(ls ${caseDir}/overlayFSDir/overlay0_*);do
   surnamesList[$i]=$(echo ${jj#*lay0})
   surnamesSubmitted[$i]="false"
   ((i++))
done
...
for jj in ${!surnamesList[@]}; do
   ...
   #---------
   echo "#6.3 Sending recursive job for file overlay0${surnameTag}"
   if [ $countRec -gt 0 ]; then
      echo "YES Submitting reconstruction for results in: ./overlayFSDir/overlay0${surnameTag} file" 
      echo "With job-name=missRec${surnameTag}"
      cd $SLURM_SUBMIT_DIR
      reconstructTimes="all"
      sbatch --job-name=missRec${surnameTag} \
             --export="surnameTag=${surnameTag},reconstructTimes=${reconstructTimes}" \
             --clusters=zeus \
             ${SLURM_SUBMIT_DIR}/E.0.reconstruct-recursiveManager.sh
      surnamesSubmitted[$jj]="true"
   else
...
#========================
echo "#X. Final step"
echo "Overlay files which were submitted to further reconstructions:"
for jj in ${!surnamesList[@]}; do
   surnameTag=${surnamesList[jj]}
   if [ "${surnamesSubmitted[jj]}" == "true" ]; then
