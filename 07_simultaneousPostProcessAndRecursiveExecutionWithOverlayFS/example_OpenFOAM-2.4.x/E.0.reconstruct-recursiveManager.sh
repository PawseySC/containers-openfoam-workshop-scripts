...
#SBATCH --job-name=reconstruct-recuManager
   ...
   if [ $nPhrase2 -gt 0 ]; then
      ...
      if [ $nSlurms -gt 2 ]; then
         ...
         if [ $nPhrase3 -gt 0 ]; then
            ...
            echo "So will continue the recursive cycle and check again for errors at the beginning of the next job"
         fi
      else
...
mainScript=$SLURM_SUBMIT_DIR/E.1.reconstructFromOverlay.sh
