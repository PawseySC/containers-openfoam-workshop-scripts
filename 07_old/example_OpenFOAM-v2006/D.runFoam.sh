...
srun -n $SLURM_NTASKS -N $SLURM_JOB_NUM_NODES bash -c "singularity exec --overlay ${overlayFSDir}/"'overlay${SLURM_PROCID}'" $theImage pimpleFoam -parallel $foam_options 2>&1" | tee $logsDir/log.pimpleFoam.$SLURM_JOBID
...
if [ -z "$reconstructTimes" ]; then
   ...
   replace="false"
   copyResultsIntoBak "$overlayFSDir" "$insideDir" "$foam_numberOfSubdomains" "$replace" "${arrayReconstruct[@]}";success=$? #Calling the function to copy time directories into bak.processor*
   if [ $success -ne 0 ]; then
