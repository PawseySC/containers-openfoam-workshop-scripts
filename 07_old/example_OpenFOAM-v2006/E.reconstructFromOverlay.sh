...
if [ $countRec -gt 0 ]; then
   ...
   while [ $kStart -lt ${#realToDoReconstruct[@]} ]; do
      ...
      replace="true"
      copyResultsIntoBak "$overlayFSDir" "$insideDir" "$foam_numberOfSubdomains" "$replace" "${arrayCopyIntoBak[@]}";success=$? #Calling the function to copy time directories into bak.processor*
      if [ $success -ne 0 ]; then
      ...
      srun -n 1 -N 1 singularity exec $theImage reconstructPar -time ${timeString} $foam_options 2>&1 | tee $logsDir/log.reconstructPar.$SLURM_JOBID.${hereToDoReconstruct[0]}
